...
public:
  ...
  void show(int W, int H, ColorTable &ct, double meanVal = -1.0) {
    switch2D::go(W, H);
    glLineWidth(1.0f);
    ...
    if (labels.empty()) {
      char strValMin[64];
      char strValMax[64];
      snprintf(strValMin, 64, "%.3E", ct.getMin());
      snprintf(strValMax, 64, "%.3E", ct.getMax());
      glText::print(xpos + wbox + 4, (int)floor(bottom) - 7, strValMin);
      glText::print(xpos + wbox + 4, (int)floor(bottom) + hbox - 7, strValMax);

      // Draw mean value if provided (>= 0)
      if (meanVal >= 0.0f) {
        char strValMean[64];
        snprintf(strValMean, 64, "%.3E", meanVal);
        // Calculate vertical position based on meanVal's position in the range
        float range = ct.getMax() - ct.getMin();
        if (range > 0.0f) {
          float normalizedPos = (meanVal - ct.getMin()) / range;
          int yPosText = (int)floor(bottom + normalizedPos * (float)hbox - 6);
          glText::print(xpos + wbox + 4, yPosText, strValMean);
        }
      }
    ...
    glText::print(xpos, H - ypos + 13, title);
